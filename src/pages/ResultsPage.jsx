 ...
 * @typedef {Object} ResultsPageProps
 * @property {Function} onRestart - Callback function to navigate the user back to the main menu or settings.
 */

/**
 * A page component that displays the history of game results for the current session.
 * It retrieves the match history from the global `useGameStore` and maps through it 
 * to render individual `ResultInfo` cards. Provides a fallback UI if no games have been played.
 * 
 * @component
 * @category Pages
 * @param {ResultsPageProps} props - The component properties.
 * @returns {JSX.Element} The rendered results history page or an empty state message.
 */
export default function ResultsPage({ onRestart }) {

  /** * Selects the game history array from the Zustand store.
   * Each history object typically contains: { id, winner, time, botLevel, mode }.
   * @type {Array}
   */
  const history = useGameStore((state) => state.history);

  // Fallback UI for when the user visits the results page without playing any matches
